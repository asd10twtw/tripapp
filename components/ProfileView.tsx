...
export const ProfileView: React.FC<ProfileViewProps> = ({ user, trips }) => {
  ...
  useEffect(() => {
    ...
    const fetchSpending = async () => {
      ...
      try {
        ...
        total = spendingResults.reduce((acc, val) => acc + val, 0);
  ...
  const getThemeStyles = () => {
    switch (user.profileTheme) {
      ...
      case 'handdrawn':
        return {
          container: 'bg-[#F9F5E6]',
          header: 'bg-[#F9F5E6] border-b border-[#4B3F35]/10',
  ...
  return (
    <div className={`flex-1 overflow-y-auto no-scrollbar pb-32 pt-12 relative ${styles.container} ${styles.font}`}>
      {user.profileTheme === 'handdrawn' && (
        <div className="absolute inset-0 pointer-events-none overflow-hidden z-0 opacity-15">
          <div className="absolute top-20 right-10 rotate-12 text-[#4B3F35]">
            <Sparkles size={48} strokeWidth={1.5} />
          </div>
          <div className="absolute bottom-40 left-10 -rotate-12 text-[#4B3F35]">
            <Tent size={48} strokeWidth={1.5} />
          </div>
          <div className="absolute top-1/2 left-4 rotate-[-15deg] text-[#4B3F35]">
            <Camera size={36} strokeWidth={1.5} />
          </div>
          <div className="absolute top-40 left-20 rotate-[20deg] text-[#4B3F35]">
            <Pencil size={32} strokeWidth={1.5} />
          </div>
          <div className="absolute bottom-20 right-20 rotate-[-10deg] text-[#4B3F35]">
            <Compass size={40} strokeWidth={1.5} />
          </div>
        </div>
      )}
      {user.profileTheme === 'handdrawn' && (
        <div className="fixed inset-0 pointer-events-none opacity-[0.12] z-0 overflow-hidden">
          <div className="absolute top-20 right-10 rotate-12 text-rose-500">
            <Heart size={48} strokeWidth={1.5} fill="currentColor" />
          </div>
          <div className="absolute top-40 left-10 rotate-[-15deg] text-amber-500">
            <Sparkles size={56} strokeWidth={1.5} />
          </div>
          <div className="absolute bottom-40 right-14 rotate-[10deg] text-sky-500">
            <Compass size={64} strokeWidth={1.5} />
          </div>
          <div className="absolute bottom-20 left-12 rotate-[5deg] text-emerald-500">
            <MapPin size={52} strokeWidth={1.5} />
          </div>
          <div className="absolute top-1/2 left-1/4 -translate-y-1/2 rotate-[-10deg] text-indigo-500">
      ...
      <div className="px-6 pt-12 pb-6">
        ...
        >
          {user.profileTheme === 'handdrawn' && (
            <>
              <div className="absolute top-4 right-6 text-rose-400/40 rotate-12"><Heart size={22} fill="currentColor" /></div>
              <div className="absolute bottom-6 right-8 text-sky-400/30 rotate-6"><Sparkles size={20} /></div>
            </>
          )}
          ...
          <div className={`flex relative z-10 ${user.profileTheme === 'hipster' ? 'flex-col items-start gap-8' : 'items-center gap-6'}`}>
            <div className="relative shrink-0">
              ...
              <div className={`
                ${user.profileTheme === 'handdrawn' ? 'w-36 h-40 rounded-none border-[1.5px] border-[#4B3F35]/10 p-2 pb-14 bg-white shadow-[4px_4px_15px_rgba(0,0,0,0.05)] overflow-hidden rotate-[-2deg]' :
                  user.profileTheme === 'hipster' ? 'w-24 h-32 rounded-none border border-stone-100 p-0.5 bg-white shadow-sm overflow-hidden' :
            ...
            <div className="flex-1 min-w-0">
              <div className={`flex items-center gap-2 ${user.profileTheme === 'handdrawn' ? 'justify-start pt-6' : 'justify-between'}`}>
                <h1 className={`font-black tracking-tight truncate ${
                  user.profileTheme === 'handdrawn' ? 'text-xl text-[#4B3F35] font-handdrawn' : 
                  user.profileTheme === 'hipster' ? 'text-3xl text-stone-700 font-hipster' :
              ...
              {user.location && (
                <div className={`flex items-center gap-1 mt-1 font-bold ${
                  ...
                  user.profileTheme === 'hipster' ? 'text-stone-400 text-[9px] font-hipster tracking-[0.1em]' :
              ...
              <p className={`mt-3 text-xs font-medium leading-relaxed ${
                ...
                user.profileTheme === 'hipster' ? 'text-stone-500 font-hipster italic border-l border-stone-100 pl-3 mt-4' :
      ...
      <div className="px-6 pb-8">
        <div className={`grid ${user.profileTheme === 'handdrawn' ? 'grid-cols-2 gap-4' : 'grid-cols-2 gap-3'}`}>
          ...
          <StatCard 
            ...
            label={user.profileTheme === 'handdrawn' ? "查看個人消費" : "個人消費"} 
            theme={user.profileTheme}
          />
        </div>
      </div>
          ...
          <div className="fixed inset-0 z-[100] bg-black/30 backdrop-blur-sm flex items-center justify-center p-6">
            ...
            >
              ...
              <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2 no-scrollbar">
                ...
                <div>
                  <label className="text-[8px] font-black text-slate-300 uppercase tracking-widest block ml-1 mb-1">興趣標籤</label>
                  <input 
                    type="text" 
                    value={editInterests}
                    onChange={e => setEditInterests(e.target.value)}
                    placeholder="例如：咖啡、攝影"
                    className="w-full p-3 bg-slate-50 rounded-2xl text-sm font-bold border-none outline-none focus:ring-1"
                  />
                </div>
                <div>
